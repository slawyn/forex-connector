...
class Chart():
    ...
    def draw_prices(self, maxprice, minprice):
        self.draw.rectangle([(Chart.CHART_OFFSET_PRICESX+10,Chart.CHART_OFFSET_TOPY),(Chart.CHART_SIZEX, Chart.CHART_SIZEY - Chart.CHART_OFFSET_BOTTOMY + Chart.CHART_OFFSET_TOPY)], fill=None, outline=Chart.COLOR_PRICES,width=2)
        precision = 20
    ...
    def generate_chart(self, dir, id, data, limits, symbol, deals):
        ...
        price_max, price_min, volume_max, volume_min = get_min_max(rates)
        ...
        for rate in rates:
            open = self.calculate_bar_y_coordinate(rate[1]-price_min)
            close = self.calculate_bar_y_coordinate(rate[4]-price_min)
            high = self.calculate_bar_y_coordinate(rate[2]-price_min)
            low = self.calculate_bar_y_coordinate(rate[3]-price_min)
